...
def admin_dashboard(request):
    ...
    context['total_plumbers'] = Account.objects.filter(role="plumber").count()
...
@login_required(login_url='login_view')
def get_all_plumbers(request):
    context = {}
    plumbers = Account.objects.filter(role="plumber")
    context['plumbers'] = plumbers
    return render(request, 'jobs/allPlumbers.html', context)
...
def assign_job(request, id, job_id):
    ...
    send_mail('Bid Assignment', message, 'sender email',
              ['to plumber email'], fail_silently=False)
    return redirect('client_dashboard')
